...
public class NextBikeXMLDataFetcher implements NextBikeDataFetcher{
    ...
    public Set<Bike> getBikes() {
        ...
        try {
            ...
            String firstId = list.item(0).getAttributes().getNamedItem("uid").getNodeValue();
            for(int i =0; i<list.getLength(); i++){
                Node location = list.item(i);
                double longitude = Double.parseDouble(location.getAttributes().getNamedItem("lng").getNodeValue());
                double latitude = Double.parseDouble(location.getAttributes().getNamedItem("lat").getNodeValue());
